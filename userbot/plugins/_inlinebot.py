...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
        ...
        if query.startswith("**Catuserbot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    ...
                    Button.url("Repo", "https://github.com/ot/catuserbot"),
                )
            ]
            ...
            result = builder.article(
                "© Userbot Help",
                text="{}\nJumlah plugin yang ada : {}".format(query, len(CMD_LIST)),
                buttons=buttons,
                link_preview=False,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                ...
                text=f"🔒 Sebuah pesan rahasia untuk {sandy}, Hanya dia yang dapat membuka ini",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Mohon gunakan ubot anda sendiri, dan jangan gunakan saya ! Hub @Hyoneechan untuk info lengkap"
        ...
        if os.path.exists("./userbot/secrets.txt"):
            ...
            try:
                ...
                if event.query.user_id in ids:
                    ...
                    reply_pop_up_alert = "Kenapa kamu sangat kepo, dasar anak haram"
            except KeyError:
                reply_pop_up_alert = "pesan ini sudah tidak ada di server bot"
        else:
            reply_pop_up_alert = "Pesan ini sudah tidak ada "
        await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    if len(pairs) > number_of_rows:
        pairs = pairs[
            ...
            (
                ...
                custom.Button.inline("Close", data="tutup"),
                custom.Button.inline(
